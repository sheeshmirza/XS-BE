...
const { getPublishAdapter } = require('../adapters/publish');
const { addSchedulePublishJob } = require('../queue/postQueue');
...
class PostService {
  ...
  async updatePost(userId, postId, payload) {
    const updated = await postRepository.updateByIdAndUserId(postId, userId, payload);
    if (!updated) {
      throw new ApiError(httpStatus.NOT_FOUND, 'Post not found');
    }
  ...
  async publishPost(userId, postId, platformList = []) {
    ...
    if (!selectedPlatforms.length) {
      throw new ApiError(httpStatus.BAD_REQUEST, 'No platform selected for publishing');
    }

    const handles = await socialService.listHandlesForPlatforms(userId, selectedPlatforms);
    if (!handles.length) {
      throw new ApiError(httpStatus.BAD_REQUEST, 'No connected social handles available for selected platforms');
    }
  ...
  async schedulePost(userId, postId, scheduledTime, timezone) {
    ...
    if (new Date(scheduledTime).getTime() <= Date.now()) {
      throw new ApiError(httpStatus.BAD_REQUEST, 'Scheduled time must be in the future');
    }
  ...
  async statusBreakdown(userId) {
    const rows = await postRepository.aggregateDashboard(new mongoose.Types.ObjectId(userId));
  ...
  async platformBreakdown(userId) {
    const rows = await postRepository.platformStats(new mongoose.Types.ObjectId(userId));
