...
class AuthService { async signup({ email, password }) { const exists = await userRepository.findByEmail(email);
  ...
  async verifyEmail(token) { const user = await userRepository.findByVerificationToken(token);
    ...
    await user.save();
    return user; }
  async login({ email, password }, metadata) { const user = await userRepository.findByEmail(email);
  ...
  async refresh(rawRefreshToken, metadata) { const rotated = await tokenService.rotateRefreshToken(rawRefreshToken, metadata);
    if (!rotated) { throw new ApiError(httpStatus.UNAUTHORIZED, 'Invalid refresh token'); }
    const user = await userRepository.findById(rotated.userId);
    if (!user) { throw new ApiError(httpStatus.UNAUTHORIZED, 'Invalid refresh token user'); }
    return { user,
      accessToken: rotated.accessToken,
      refreshToken: rotated.refreshToken }; }
  async logout(rawRefreshToken, userId) { if (rawRefreshToken) { await tokenService.revokeRefreshToken(rawRefreshToken); }
    if (userId) { await tokenService.revokeAllForUser(userId); } }
  async forgotPassword(email) { const user = await userRepository.findByEmail(email);
    if (!user) return;
  ...
  async resetPassword(token, newPassword) { const user = await userRepository.findByResetToken(token);
    ...
    await userRepository.updateById(user._id, { password: hash,
