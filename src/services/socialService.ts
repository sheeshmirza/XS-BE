...
class SocialService { async listAccounts(userId, filters, options) { const dbFilters: any = {};
    ...
    return { items, total }; }
  getConnectUrl(userId, platform) { if (!allowedPlatforms.includes(platform)) { throw new ApiError(httpStatus.BAD_REQUEST, 'Unsupported platform'); }
  ...
  async connectFromCallback(userId, platform, code) { const adapter = getOAuthAdapter(platform);
    if (!adapter) { throw new ApiError(httpStatus.BAD_REQUEST, 'OAuth adapter not found for platform'); }
  ...
  async refreshPlatformToken(userId, socialId) { const social: any = await socialHandleRepository.findByIdAndUserId(socialId, userId);
    ...
    if (!adapter) { throw new ApiError(httpStatus.BAD_REQUEST, 'OAuth adapter not found for platform'); }
    if (!social.refreshToken) { throw new ApiError(httpStatus.BAD_REQUEST, 'No refresh token available for this social account'); }
    const refreshed = await adapter.refreshToken(social.refreshToken);
    if (!refreshed || !refreshed.access_token) { await socialHandleRepository.updateByIdAndUserId(social._id, userId, { isConnected: false });
      await notificationService.createNotification({ userId,
    ...
    return socialHandleRepository.updateByIdAndUserId(social._id, userId, { accessToken: refreshed.access_token,
      ...
      tokenExpiry: refreshed.expires_in
        ...
        : social.tokenExpiry,
      isConnected: true }); }
  listHandlesForPlatforms(userId, platformList) { return socialHandleRepository.findByUserPlatforms(userId, platformList); }
  countConnected(userId) { return socialHandleRepository.countByUserId(userId, { isConnected: true }); } }
