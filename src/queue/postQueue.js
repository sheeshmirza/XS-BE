...
const { processPublishPostJob } = require('../jobs/publishPostJob');

let postQueue;
let worker;

const initializePostQueue = async () => {
  if (!env.redisUrl) {
  ...
  worker.on('failed', (job, error) => {
    logger.error({ message: 'Post publish job failed', jobId: job?.id, error: error.message });
  });

  logger.info('BullMQ post queue initialized');
};

const addSchedulePublishJob = async ({ postId, runAt }) => {
  if (!postQueue) {
    throw new Error('Post queue is not initialized. Ensure REDIS_URL is configured.');
  }
