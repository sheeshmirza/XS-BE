...
const uploadMedia = asyncHandler(async (req, res) => {
  ...
  if (!hasCloudinaryConfig) {
    throw new ApiError(httpStatus.BAD_REQUEST, 'Cloudinary is not configured in environment variables');
  }
  ...
  for (const file of req.files) {
    ...
    media.push({
      type: file.mimetype.startsWith('video/') ? 'video' : file.mimetype === 'image/gif' ? 'gif' : 'image',
