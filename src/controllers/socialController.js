...
const listAccounts = asyncHandler(async (req, res) => {
  ...
  return sendSuccess(res, httpStatus.OK, 'Social accounts fetched', result.items, {
    page,
    limit,
    total: result.total
  });
...
const connectPlatform = asyncHandler(async (req, res) => {
  const authUrl = socialService.getConnectUrl(req.user._id.toString(), req.params.platform);
...
const refreshSocialToken = asyncHandler(async (req, res) => {
  const socialHandle = await socialService.refreshPlatformToken(req.user._id, req.body.socialId);
