...
const mediaSchema = Joi.object({ type: Joi.string().valid('image', 'video', 'gif').required(),
  url: Joi.string().uri().required(),
  mimeType: Joi.string().optional(),
  size: Joi.number().min(0).optional() });
const createPost = Joi.object({ body: Joi.object({ title: Joi.string().max(140).allow('').optional(),
...
const listPosts = Joi.object({ body: Joi.object({}).required(),
  params: Joi.object({}).required(),
...
const postIdParam = Joi.object({ body: Joi.object({}).required(),
  params: Joi.object({ id: Joi.string().required() }).required(),
  query: Joi.object({}).required() });
const updatePost = Joi.object({ body: Joi.object({ title: Joi.string().max(140).allow('').optional(),
...
const schedulePost = Joi.object({ body: Joi.object({ scheduledTime: Joi.date().iso().required(),
    timezone: Joi.string().optional() }).required(),
  params: Joi.object({ id: Joi.string().required() }).required(),
  query: Joi.object({}).required() });
